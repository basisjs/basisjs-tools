...
function resolveFormat(fmt){
  ...
  console.warn('ERROR: Unknown `' + fmt + '` format for external files.');
  process.exit();
}
...
function apply(command){
  ...
  return command
    ...
    .usage('basis l10n [export|import <filename>]')
    ...
    .option('--format <format>', 'External file format', resolveFormat, 'json')
...
function norm(options){
  ...
  switch (options.topic)
  {
    ...
    case 'import':
      ...
      {
        console.error('No filename for import');
        process.exit();
      }

      options.output = path.resolve(options.output);
      options.importFilename = path.resolve(options.importFilename)
