// patched gonsalez from csso 1.3.11
// changes:
//   - save column into info in pushToken
//   - use token.info when build ast token
//   - support for /deep/ combinator
...
var getTokens = (function() {
    ...
    function pushToken(type, value) {
        var col = lastPos ? lastPos - source.lastIndexOf('\n', lastPos) + 1 : 1; // TODO: take in account \r
        tokens.push({
            tn: tn++,
            ln: ln,
            type: type,
...
//atkeyword = '@' ident:x -> [#atkeyword, x]
    ...
    function getAtkeyword() {
        ...
        return needInfo?
            [tokens[startPos].info, CSSPNodeType.AtkeywordType, getIdent()]:
...
//       | '[' sc*:s0 ident:x sc*:s1 ']' -> this.concat([#attrib], s0, [x], s1),
    ...
    function getAttrib1() {
        ...
        var a = (needInfo? [tokens[startPos].info, CSSPNodeType.AttribType] : [CSSPNodeType.AttribType])
                .concat(getSC())
                .concat([getIdent()])
                .concat(getSC())
...
//attrselector = (seq('=') | seq('~=') | seq('^=') | seq('$=') | seq('*=') | seq('|=')):x -> [#attrselector, x]
    ...
    function getAttrselector() {
        ...
        return needInfo?
                [tokens[startPos].info, CSSPNodeType.AttrselectorType, s] :
...
//atruleb = atkeyword:ak tset*:ap block:b -> this.concat([#atruleb, ak], ap, [b])
    ...
    function getAtruleb() {
        return (needInfo?
                    [tokens[pos].info, CSSPNodeType.AtrulebType, getAtkeyword()] :
...
//atruler = atkeyword:ak atrulerq:x '{' atrulers:y '}' -> [#atruler, ak, x, y]
    ...
    function getAtruler() {
        var atruler = needInfo?
                        [tokens[pos].info, CSSPNodeType.AtrulerType, getAtkeyword(), getAtrulerq()] :
...
//atrulerq = tset*:ap -> [#atrulerq].concat(ap)
    ...
    function getAtrulerq() {
        return (needInfo? [tokens[pos].info, CSSPNodeType.AtrulerqType] : [CSSPNodeType.AtrulerqType]).concat(getTsets());
    }
...
//atrulers = sc*:s0 ruleset*:r sc*:s1 -> this.concat([#atrulers], s0, r, s1)
    ...
    function getAtrulers() {
        var atrulers = (needInfo? [tokens[pos].info, CSSPNodeType.AtrulersType] : [CSSPNodeType.AtrulersType]).concat(getSC()),
            x;
...
//atrules = atkeyword:ak tset*:ap ';' -> this.concat([#atrules, ak], ap)
    ...
    function getAtrules() {
        var atrules = (needInfo? [tokens[pos].info, CSSPNodeType.AtrulesType, getAtkeyword()] : [CSSPNodeType.AtrulesType, getAtkeyword()]).concat(getTsets());
...
//block = '{' blockdecl*:x '}' -> this.concatContent([#block], x)
    ...
    function getBlock() {
        var block = needInfo? [tokens[pos].info, CSSPNodeType.BlockType] : [CSSPNodeType.BlockType],
...
//       | '[' tset*:x ']' -> this.concat([#braces, '[', ']'], x)
    ...
    function getBraces() {
        ...
        return needInfo?
                [tokens[startPos].info, CSSPNodeType.BracesType, tokens[left].value, tokens[right].value].concat(tsets) :
    ...
    function getClazz() {
        ...
        return needInfo?
                [tokens[startPos].info, CSSPNodeType.ClazzType, getIdent()] :
    ...
    function getCombinator() {
        return needInfo?
                [tokens[pos].info, CSSPNodeType.CombinatorType, tokens[pos++].value] :
    ...
    function getComment() {
        ...
        return needInfo?
                [tokens[startPos].info, CSSPNodeType.CommentType, s] :
    ...
    function getDeclaration() {
        var declaration = needInfo?
                [tokens[pos].info, CSSPNodeType.DeclarationType, getProperty()] :
    ...
    function getDecldelim() {
        ...
        return needInfo?
                [tokens[startPos].info, CSSPNodeType.DecldelimType] :
    ...
    function getDelim() {
        ...
        return needInfo?
                [tokens[startPos].info, CSSPNodeType.DelimType] :
    ...
    function getDimension() {
        var startPos = pos,
            n = getNumber(),
            dimension = needInfo ?
                [tokens[pos].info, CSSPNodeType.IdentType, getNmName2()] :
                [CSSPNodeType.IdentType, getNmName2()];

        return needInfo?
                [tokens[startPos].info, CSSPNodeType.DimensionType, n, dimension] :
...
//filter = filterp:x ':' filterv:y -> [#filter, x, y]
    ...
    function getFilter() {
        var filter = needInfo?
                [tokens[pos].info, CSSPNodeType.FilterType, getFilterp()] :
...
//filterp = (seq('-filter') | seq('_filter') | seq('*filter') | seq('-ms-filter') | seq('filter')):t sc*:s0 -> this.concat([#property, [#ident, t]], s0)
    ...
    function getFilterp() {
        var startPos = pos,
            ...
            ident = needInfo? [tokens[startPos].info, CSSPNodeType.IdentType, x] : [CSSPNodeType.IdentType, x];

        pos += tokens[pos].filterp_l;

        return (needInfo? [tokens[startPos].info, CSSPNodeType.PropertyType, ident] : [CSSPNodeType.PropertyType, ident])
                    .concat(getSC());
...
//filterv = progid+:x -> [#filterv].concat(x)
    ...
    function getFilterv() {
        var filterv = needInfo? [tokens[pos].info, CSSPNodeType.FiltervType] : [CSSPNodeType.FiltervType],
...
//functionExpression = ``expression('' functionExpressionBody*:x ')' -> [#functionExpression, x.join('')],
    ...
    function getFunctionExpression() {
        ...
        return needInfo?
                [tokens[startPos].info, CSSPNodeType.FunctionExpressionType, e] :
...
//funktion = ident:x '(' functionBody:y ')' -> [#funktion, x, y]
    ...
    function getFunktion() {
        ...
        return needInfo?
                [tokens[startPos].info, CSSPNodeType.FunktionType, ident, body] :
    ...
    function getFunctionBody() {
        ...
        return (needInfo?
                    [tokens[startPos].info, CSSPNodeType.FunctionBodyType] :
    ...
    function getIdent() {
        ...
        return needInfo?
                [tokens[startPos].info, CSSPNodeType.IdentType, s] :
...
//important = '!' sc*:s0 seq('important') -> [#important].concat(s0)
    ...
    function getImportant() {
        ...
        return (needInfo? [tokens[startPos].info, CSSPNodeType.ImportantType] : [CSSPNodeType.ImportantType]).concat(sc);
    }
    ...
    function getNamespace() {
        ...
        return needInfo?
                [tokens[startPos].info, CSSPNodeType.NamespaceType] :
...
//    | (seq('even') | seq('odd')):x -> [#nth, x]
    ...
    function getNth() {
        ...
        if (tokens[pos].nth_last) {
            var n = needInfo?
                        [tokens[startPos].info, CSSPNodeType.NthType, joinValues(pos, tokens[pos].nth_last)] :
        ...
        return needInfo?
                [tokens[startPos].info, CSSPNodeType.NthType, tokens[pos++].value] :
...
//nthselector = nthf:x '(' (sc | unary | nth)*:y ')' -> [#nthselector, [#ident, x]].concat(y)
    ...
    function getNthselector() {
        var startPos = pos,
            nthf = needInfo?
                    [tokens[pos].info, CSSPNodeType.IdentType, getNthf()] :
                    [CSSPNodeType.IdentType, getNthf()],
            ns = needInfo?
                    [tokens[pos].info, CSSPNodeType.NthselectorType, nthf] :
    ...
    function getNumber() {
        ...
        return needInfo?
                [tokens[startPos].info, CSSPNodeType.NumberType, s] :
    ...
    function getOperator() {
        return needInfo?
                [tokens[pos].info, CSSPNodeType.OperatorType, tokens[pos++].value] :
    ...
    function getPercentage() {
        ...
        return needInfo?
                [tokens[startPos].info, CSSPNodeType.PercentageType, n] :
...
//                -> this.concat([#progid], s0, [[#raw, x + y.join('') + '(' + z.join('') + ')']], s1),
    ...
    function getProgid() {
        ...
        return (needInfo? [tokens[startPos].info, CSSPNodeType.ProgidType] : [CSSPNodeType.ProgidType])
                .concat(getSC())
    ...
    function _getProgid(progid_end) {
        ...
        return needInfo?
                [tokens[startPos].info, CSSPNodeType.RawType, x] :
...
//property = ident:x sc*:s0 -> this.concat([#property, x], s0)
    ...
    function getProperty() {
        ...
        return (needInfo?
                [tokens[startPos].info, CSSPNodeType.PropertyType, getIdent()] :
    ...
    function getPseudoe() {
        ...
        return needInfo?
                [tokens[startPos].info, CSSPNodeType.PseudoeType, getIdent()] :
...
//pseudoc = ':' (funktion | ident):x -> [#pseudoc, x]
    ...
    function getPseudoc() {
        ...
        return needInfo?
                [tokens[startPos].info, CSSPNodeType.PseudocType, checkFunktion(pos)? getFunktion() : getIdent()] :
    ...
    function getRuleset() {
        var ruleset = needInfo? [tokens[pos].info, CSSPNodeType.RulesetType] : [CSSPNodeType.RulesetType];
    ...
    function getS() {
        ...
        return needInfo? [tokens[startPos].info, CSSPNodeType.SType, s] : [CSSPNodeType.SType, s];
    }
    ...
    function getSelector() {
        var selector = needInfo? [tokens[pos].info, CSSPNodeType.SelectorType] : [CSSPNodeType.SelectorType],
    ...
    function getShash() {
        ...
        return needInfo?
                [tokens[startPos].info, CSSPNodeType.ShashType, getNmName()] :
...
//simpleselector = (nthselector | combinator | attrib | pseudo | clazz | shash | any | sc | namespace)+:x -> this.concatContent([#simpleselector], [x])
    ...
    function getSimpleSelector() {
        var ss = needInfo? [tokens[pos].info, CSSPNodeType.SimpleselectorType] : [CSSPNodeType.SimpleselectorType],
            t;
    ...
    function getString() {
        ...
        return needInfo?
                [tokens[startPos].info, CSSPNodeType.StringType, tokens[pos++].value] :
    ...
    function getStylesheet(_i) {
        var t,
            stylesheet = needInfo? [tokens[pos].info, CSSPNodeType.StylesheetType] : [CSSPNodeType.StylesheetType];
...
//tset = vhash | any | sc | operator
    ...
    function getUnary() {
        ...
        return needInfo?
                [tokens[startPos].info, CSSPNodeType.UnaryType, tokens[pos++].value] :
    ...
    function getUnknown() {
        ...
        return needInfo?
                [tokens[startPos].info, CSSPNodeType.UnknownType, tokens[pos++].value] :
...
//        | seq('url(') sc*:s0 (~')' ~m_w char)*:x sc*:s1 ')' -> this.concat([#uri], s0, [[#raw, x.join('')]], s1),
    ...
    function getUri() {
        ...
        if (checkUri1(pos)) {
            var uri = (needInfo? [tokens[startPos].info, CSSPNodeType.UriType] : [CSSPNodeType.UriType])
                        .concat(getSC())
                        .concat([getString()])
                        .concat(getSC());
            ...
            var uri = (needInfo? [tokens[startPos].info, CSSPNodeType.UriType] : [CSSPNodeType.UriType])
                ...
                raw = needInfo?
                        [tokens[pos].info, CSSPNodeType.RawType, joinValues(pos, pos + l)] :
    ...
    function getValue() {
        var ss = needInfo? [tokens[pos].info, CSSPNodeType.ValueType] : [CSSPNodeType.ValueType],
            t;
    ...
    function getVhash() {
        ...
        return needInfo?
                [tokens[startPos].info, CSSPNodeType.VhashType, getNmName2()] :
