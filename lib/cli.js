...
function parseParams(str){
  ...
  // [foo]..[bar] - 
  var result = false;
  var left = str.trim().replace(/^(?:<([a-zA-Z][a-zA-Z0-9\-\_]*)>(?:\.\.<([a-zA-Z][a-zA-Z0-9\-\_]*)>)?|\[([a-zA-Z][a-zA-Z0-9\-\_]*)\](?:\.\.\[([a-zA-Z][a-zA-Z0-9\-\_]*)\])?)$/, function(m, req_start, req_end, opt_start, opt_end){
    result = {};
    ...
    if (req_start)
    {
      ...
      result.start = req_start;
      result.end = req_end || '';
    }
    else
    {
      // [argument]
      result.start = opt_start;
      result.end = opt_end || '';
    }
...
var Option = function(usage, description){
  ...
  try {
    ...
    throw new Error('Bad paramenter description in usage for option: ' + this.usage);
  }
  ...
  for (var i = 0; i < args.length; i++)
  {
    ...
    else
    {
      if (command.commands[token])
      {
        if (commandArgs.length)
        {
          command.args_.apply(command, [command].concat([commandArgs.slice(0)]));
          commandArgs = [];
        }
      ...
      {
        if (noOptionsYet && command.params && commandArgs.length < command.params.argCount)
        {
          commandArgs.push(token);
          continue;
        }
