  ...
  */ 
  ...
  File.prototype = {
    ...
    save: function(content){
      ...
      var self = this;
      sendToServer('saveFile', this.data.filename, content, true, DEBUG && function(err){
        if (err)
  ...
  function loadFiles(filelist){
    for (var i = 0, data; data = filelist[i]; i++)
      if (data.type == 'file')
  ...
  function createFile(filename, content){
    return fileMap[filename] || new File({
      filename: filename,
      content: content
    });
  }
  ...
  if (typeof basis != 'undefined' && basis.data)
  {
    ...
    var existsCheck = !!basis.resource.exists && basis.resource.exists.length == 2;
    ...
    var BasisDataFile = DataObject.subclass({
      ...
      read: function(){
        ...
        sendToServer('readFile', this.data.filename, function(data){
          self.update({ content: data.content });
      ...
      emit_update: function(delta){
        ...
        if ('filename' in delta || 'content' in delta)
        {
          ...
          if (!existsCheck || basis.resource.exists(this.data.filename))
  ...
  var event = new CustomEvent('basisjs-tools-file-sync-inited');
