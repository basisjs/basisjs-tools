...
function applyConfig(command, config, configPath){
  ...
  if (!config.editor)
  {
    var editor = command.context.globalConfig && command.context.globalConfig.get('editor');
    if (editor)
      command.values.editor = editor;
  }
...
module.exports = clap.create('server')
  ...
  .init(function(){
    ...
    if (config)
    {
      ...
      if (config.plugins)
      {
        this.values.plugins = config.plugins;
        this.values.configPath_ = this.context.configPath;
      }
    }
  })
  ...
  .action(function(){
    if (this.values.verbose && this.context.configFile)
