...
module.exports = function(options){
  return function(filename, callback){
    ...
    {
      logWarn('socket', 'getAppProfile: callback not a function');
      return;
    }
    ...
    if (!fn)
    {
      logWarn('socket', 'getAppProfile: file is not resolved (' + filepath + ')');
    ...
    require('basisjs-tools-build').extract
      ...
      .on('message', function(res){
        if (res.error)
        {
          logWarn('socket', 'send error on getAppProfile: ' + res.error);
