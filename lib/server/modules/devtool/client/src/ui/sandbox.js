...
var Frame = Node.subclass({
  ...
  initUI: function(){
    if (this.ready && (this.url || this.script) && this.element)
    {
      ...
      contentWindow.eval((this.script || '') + ';console.log("Remote devtool client inited");');
    }
  },

  init: function(){
    Node.prototype.init.call(this);
    this.socketId = basis.fn.publicCallback(createSandboxAPI.bind(this, this.clientId));
  },
  destroy: function(){
      ...
      transport.getClientUI(this.data.id, function(err, type, content){
        ...
        if (err)
          ...
          this.setSatellite('frame', new Frame({
            ...
            script: type === 'script' ? content : null
          }));
      }.bind(this));
    }
  }
