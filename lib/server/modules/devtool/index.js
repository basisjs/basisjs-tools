...
module.exports = function initFileSync(wsServer, options){
  ...
  function stopIdentify(){
    ...
    for (var id in clients)
      clients[id].socket.emit('devtool:stop identify');
  }
  ...
  wsServer.on('connect', function(socket){
    ...
    socket.on('devtool:pickClient', function(){
      pickClientSocket = socket;
      mode = 'identify';
      lastNum = 1;
      for (var id in clients)
      {
        clients[id].num = lastNum++;
        clients[id].socket.emit('devtool:identify', clients[id].num);
      }
    });
    ...
    socket.on('devtool:getClientUI', function(clientId, callback){
