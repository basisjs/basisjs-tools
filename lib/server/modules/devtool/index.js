...
module.exports = function initDevtool(wsServer, httpServer, options){
  ...
  wsServer.on('connect', function(socket){
    ...
    socket.on('devtool:client connect', function(data, connectCallback){
      ...
      this
        ...
        .on('devtool:session data', function(data){
          socket.to('session-' + client.id).emit('devtool:session data', data);
        })
    ...
    socket.on('devtool:inspector connect', function(connectCallback){
      ...
      this.on('devtool:get client ui', function(clientId, callback){
        ...
        client.emit('devtool:get ui', options.dev, callback);
      });
    ...
    socket.on('devtool:join session', function(clientId, callback){
      ...
      if (!client || !client.socket)
      {
        callback('[devtool:join session] Client (' + clientId + ') not found or disconnected');
        return;
      }

      this.join('session-' + client.id);
      this.on('devtool:session command', function(data){
        client.emit('devtool:session command', data);
      });
    });
  });
  ...
  {
    logMsg('ws', 'init ' + chalk.yellow('dev version') + ' of ' + chalk.yellow('devtool'));
  ...
  {
    logMsg('ws', 'init ' + chalk.green('build version') + ' of ' + chalk.yellow('devtool'));
