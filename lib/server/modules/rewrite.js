...
function createRewriteFunction(rulesMap){
  return function(loc, req, res){
    function getRulesPath(){
      return pathRules != '*' ? ' <' + chalk.green(pathRules) + '>' : '';
    }

    var referer = req.headers.referer ? url.parse(req.headers.referer, true, true).pathname : '';
    var pathRules = Object.keys(rulesMap)
                      .filter(function(path){
                        return referer.indexOf(path) === 0;
                      })
                      .sort(function(a, b){
                        return a.length - b.length;
                      })[0] || '*';
    var rules = rulesMap[pathRules];
    ...
    for (var key in config)
    {
      ...
      if (typeof value != 'string')
      {
        ...
        for (var skey in value)
          processRule(rewriteRules[key], skey, value[skey]);
      }
      else
      {
        processRule(rewriteRules['*'], key, value);
      }
    }

    result = createRewriteFunction(rewriteRules);
