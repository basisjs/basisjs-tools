...
require('./virtualPath').add('/basisjs-tools/resourceCache.js', function(api){
...
module.exports = {
  ...
  add: function(fnKey, content){
    ...
    {
      logMsg('cache', fnKey + ' ' + (fnKey in cache ? chalk.yellow('(update)') : chalk.green('(add)')), true);
      cache[fnKey] = content;
  ...
  remove: function(fnKey){
    if (fnKey in cache)
    {
      logMsg('cache', fnKey + ' ' + chalk.red('(drop)'), true);
      delete cache[fnKey];
