...
function getFileAttrs(file){
  ...
  if (resourceCache.has(relativePath(file.filename)))
    attrs.push('cache');
  if (file.watching)
    attrs.push('watch');
  if (file.notify)
...
function addToCache(filename, data){
...
module.exports = {
  ...
  readFileIfNeeded: function(filename, callback){
    if (processingPaths[filename] || !getFileInfo(filename) || getFileInfo(filename).content === null)
    {
