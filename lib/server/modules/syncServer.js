...
module.exports = function createServer(httpServer, options){
  ...
  syncServer.sockets.on('connection', function(socket){
    ...
    socket.on('saveFile', require('./socket/saveFile.js')(options, broadcast));
    socket.on('createFile', require('./socket/createFile.js')(options, broadcast));
    socket.on('readFile', require('./socket/readFile.js')(options, broadcast));

    socket.on('openFile', require('./socket/openFile.js')(options, broadcast));
    socket.on('getFileGraph', require('./socket/getFileGraph.js')(options, broadcast));
    socket.on('handshake', require('./socket/handshake.js')(options, broadcast));
  });
