...
function isContentModified(req, res, date){
  if (req.headers['if-modified-since'])
  {
    ...
    {
      logMsg('http', req.url + ' ' + chalk.green(304));
      res.writeHead(304, {
...
module.exports = function createServer(options, fsWatcher){
  ...
  function defaultRequestHandler(req, res){
    resolvePathnameFile(req, res, function(fres){
      ...
      {
        logMsg('http', pathname + ' ' + chalk.green('304'));
        return;
      }
      ...
      if (options.readCache)
      {
        ...
        {
          logMsg('http', pathname + ' ' + chalk.green('(from cache)'));
          getFileHandler(req, res)(null, filename, file.content);
          return;
        }
      }

      logMsg('http', pathname + ' ' + chalk.yellow('(file read)'));
      files.readFileIfNeeded(filename, getFileHandler(req, res));
    });
  }

  function processRequest(req, res, middleware){
    logMsg('http', req.url + chalk.gray(' → ') + chalk.yellow(middleware.fn.name || '<noname-middleware>'));
