...
function launch(options){
    ...
    '/FDF31FF4-1532-421C-A865-99D0E77ADE04.js': function(req, res){
      ...
      if (!contentIsNotModified(req, res, content.mtime))
      {
        res.writeHead(200, {
          ...
          'Last-Modified': content.mtime.toGMTString()
        });
        res.end('window.__resources__ = ' + content);
      }
    }
  };
  ...
  function resolvePathnameFile(req, res, filename, location){
    ...
    filename.stat = fs.statSync(filename);
    if (filename.stat.isDirectory())
    {
  ...
  function contentIsNotModified(req, res, date){
    ...
    {
      var headerDate = +(new Date(req.headers['if-modified-since']));
      if (isNaN(headerDate))
      {
  ...
  var httpServer = http.createServer(function(req, res){
    ...
    if (filename)
    {
      var fileStat = filename.stat || fs.statSync(filename);
      ...
      function processFile(err, data){
        ...
        res.writeHead(200, {
          ...
          'Last-Modified': fileStat.mtime.toGMTString(),
          'Cache-Control': 'max-age=' + (60 * 60 * 1000), // 1 hour
          'Expires': (new Date(+(new Date) + 60*60*1000)).toGMTString()
        });
