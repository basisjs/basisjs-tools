...
function launch(options){
  ...
  // Proxy
  //
  function proxyRequest(req, res, loc){
    for (var i = 0, rule; rule = rewriteRules[i]; i++)
    {
      var pathmatch = loc.path.match(rule.re);
      if (pathmatch)
      {
        if (!proxy)
        {
        ...
        var location = url.parse(rule.url.replace(/\$(\d+)/g, function(m, n){
          return pathmatch[n] || m;
        }));

        proxy.target.host = location.hostname;
        proxy.target.port = location.port;
  ...
  var virtualPath = {
    '/FDF31FF4-1532-421C-A865-99D0E77ADE04.js': function(req, res){
      ...
      {
        console.log('[CLIENT]', '/FDF31FF4-1532-421C-A865-99D0E77ADE04.js', '(from cache)');
        ...
        console.log('[CLIENT]', '/FDF31FF4-1532-421C-A865-99D0E77ADE04.js', '304');
    }
  };
  ...
  function getFileHandler(fres, fnKey, req, res, fileStat, opt){
    ...
    return function(err, data){
      ...
      if (contentType == 'text/html' && (options.resCache || options.sync))
      {
        ...
        if (options.resCache)
          fileContent = fileContent.replace(/<script/i, '<script src="/FDF31FF4-1532-421C-A865-99D0E77ADE04.js"></script><script');
  ...
  //
  var appcp_server = require('./assets/appcp_server.js');
  appcp_server.create();
  ...
  var httpServer = http.createServer(function(req, res){
    ...
    if (proxyRequest(req, res, location))
      return;
