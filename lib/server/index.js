...
var fsIsCaseSensetive = fs.existsSync(process.execPath.toLowerCase()) && fs.existsSync(process.execPath.toUpperCase());


//
// launch by top command
//
exports.commandAction = function(this){
  if (this instanceof cli.Command == false)
    throw 'command should be instance of Command';

  return launch(this.values);
};
...
function launch(config){
  var options = moduleOptions.norm(config);
  ...
  io.sockets.on('connection', function(socket){
    ...
    socket.on('saveFile', function(filename, content, autocreate, callback){
      console.log('[SOCKET] request saveFile', autocreate ? '(autocreate)' : '', filename, '\n' + content);
      ...
      {
        console.log('[FS] write file', fname);
        fs.writeFile(fname, content, function(err){
          callback(err);
        });
      }
    ...
    socket.on('createFile', function(filename, content, callback){
      console.log('[SOCKET] request createFile', filename, content);
    ...
    socket.on('readFile', function(filename, callback){
      console.log('[SOCKET] request readFile', filename);
    ...
    socket.on('getFileGraph', function(filename, callback){
      console.log('[SOCKET] request getFileGraph', filename);
      ...
      if (typeof callback == 'function')
      {
        ...
        var cp = require('child_process').fork(__dirname + '/../extractor',
        ...
        cp.on('message', function(res){
          if (res.error)
          {
            console.warn('[SOCKET] send error on getFileGraph: ' + res.error);
            callback('Error on file map fetch: ' + res.error);
          }
          else
          {
            console.log('[SOCKET] send file graph (extrating done in', new Date - startTime, 'ms)');
  ...
  var createCallback = function(fileInfo){
    console.log('[SOCKET] broadcast newFile', fileInfo.filename);
    io.sockets.emit('newFile', fileInfo);
  }
  var updateCallback = function(fileInfo){
    console.log('[SOCKET] broadcast updateFile', fileInfo.filename);
    io.sockets.emit('updateFile', fileInfo);
  }
  var deleteCallback = function(filename){
    console.log('[SOCKET] broadcast deleteFile', filename);
