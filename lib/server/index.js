...
function launch(options){
  ...
  var httpServer = http.createServer(function(req, res){
    ...
    if (fres)
    {
      ...
      if (fres.isDebug)
      {
        console.log('[CLIENT] ', location.pathname, '(force file read)');
      ...
      else
      {
        if (options.readCache)
        {
          ...
          {
            console.log('[CLIENT] ', location.pathname, 304);
            return;
          }

          if (fileMap[fnKey] && fileMap[fnKey].content != null)
          {
            console.log('[CLIENT] ', location.pathname, '(from cache)');
            getFileHandler(fres, fnKey, req, res, fileStat, location.query)(null, fileMap[fnKey].content);
            return;
          }
        }
        
        console.log('[CLIENT] ', location.pathname, '(file read)');
        fs.readFile(filename, getFileHandler(fres, fnKey, req, res, fileStat, location.query));
      }
    }
  });
