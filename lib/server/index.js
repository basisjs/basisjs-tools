...
function launch(config){
  ...
  files.onRead(function(err, filename, content){
    ...
    var file = files.getInfo(filename);
    var fnKey = normPath(filename);

    if (file.content != content)
    {
      logMsg('info', fnKey + ' ' + chalk.yellow('(update content: ' + content.length + 'bytes)'), true);

      file.zip = {};
      files.updateCache(filename, content);
  ...
  function getFileHandler(fres, fnKey, req, res, fileStat, location){
    ...
    return function(err, filename_, data){
      if (err)
      {
        res.writeHead(500);
    ...
    resolvePathnameFile(req, res, path.normalize(options.base + location.pathname), location, function(fres){
      ...
      if (options.readCache)
      {
        ...
        if (fileInfo && fsWatcher.isFileObserve(filename) && fileInfo.content != null)
        {
          ...
          getFileHandler(fres, fnKey, req, res, fileStat, location)(null, null, fileInfo.content);
          return;
        }
      }
  ...
  require('./modules/sync-server')(httpServer, options);
