...
function launch(config){
  ...
  function normPath(filename){
    return '/' + path.relative(options.base, path.resolve(options.base, filename)).replace(/\\/g, '/')
  }
  ...
  io.sockets.on('connection', function(socket){
    ...
    socket.on('readFile', function(filename, callback){
      ...
      if (!fileInfo || !fileInfo.touched)
        files.readFile(path.normalize(options.base + '/' + filename), function(err, data){
          if (!err)
            return {
              filename: fnKey,
    ...
    socket.on('getFileGraph', function(filename, callback){
      ...
      if (typeof callback == 'function')
      {
        ...
        var cp = child_process.fork(__dirname + '/../extractor',
          ['--file', fn, '--base', options.base, '--js-cut-dev', '--css-info', '--l10n-info'],
          { silent: true }
        );
  ...
  var fsWatcher = (function(){
    ...
    files.addReadCallback(function(err, data, filename){
      if (!err && hasSocketClients)
        updateCallback(getFileInfo(filename));
    });
