...
function launch(config){
  ...
  function logMsg(topic, message, verbose){
    if (!verbose || options.verbose)
      console.log(chalk.cyan(ralign('[' + topic.toUpperCase() + ']', 8)) + ' ' + chalk.white(message));
  }
  ...
  var virtualPath = {
    '/__basis_resources__.js': function(req, res){
      ...
      if (!contentIsNotModified(req, res, cache.mtime))
      {
        ...
        logMsg('client', '/__basis_resources__.js' + ' ' + '(from cache)');
  ...
  io.sockets.on('connection', function(socket){
    ...
    socket.on('readFile', function(filename, callback){
      ...
      if (!fileInfo || !fileInfo.touched)
        fsWatcher.readFile(path.normalize(BASE_PATH + '/' + filename));
    });
  ...
  var fsWatcher = (function(){
    ...
    function readFile(filename){
      ...
      if (hotStartCache.isRequire(fnKey) || readExtensions.indexOf(path.extname(filename)) != -1)
      {
        ...
        files.readFile(filename, function(err, data){
          if (!err)
          {
            ...
            if (hasSocketClients && (fileInfo.notify || hotStartCache.isRequire(fnKey)))
              updateCallback(newFileInfo);

            if (hotStartCache.isRequire(fnKey))
              hotStartCache.add(fnKey, newContent);
          }
          else
            logMsg('fs', 'Error: Can\'t read file ' + filename + ': ' + err);
        });
      }
      else
      {
        if (fileInfo && fileInfo.content != null)
        {
        ...
        else
        {
          ...
          if (!fileMap[fnKey])
          {
            ...
            fileMap[fnKey] = {
              mtime: stats.mtime,
              type: fileType,
              hotStart: options.hotStartCacheByExt ? hotStartExtensions.indexOf(ext) != -1 : false,
    ...
    function lookup(dirname){
      fs.exists(dirname, function(exists){
        ...
        fs.readdir(dirname, function(err, files){
          if (err)
            return console.warn('lookup error:', dirname, err);
          ...
          if (dirInfo)
          {
            ...
            for (var i = 0, file; file = dirFiles[i++];)
            {
              if (files.indexOf(file) == -1)
              {
                ...
                logMsg('fs', 'delete file ' + fnKey); // file lost
              }
            }
          }
          else
