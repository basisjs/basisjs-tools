...
function launch(config){
  ...
  if (options.plugins.length)
  {
    ...
    // hack to resolve modules relative to basis.config
    // TODO: find better solution
    var resolver = (function(){
      var tmpFile = path.join((options.configPath_ || options.base), '.' + parseInt(Math.random() * 10e10).toString(36) + '.js');
      var result;

      fs.writeFileSync(tmpFile, 'module.exports = function(name){ return require(name); }');
    ...
    options.plugins.forEach(function(pluginCfg){
      try {
        var plugin = resolver(pluginCfg.name);

        if (typeof plugin.server == 'function')
        {
          try {
            plugin.server(require('./modules/plugin-api')(options), options);
            console.log('  ' + pluginCfg.name + chalk.green(' loaded'));
          } catch(e) {
    ...
    return function(err, data){
      ...
      if (contentType == 'text/html')
      {
        ...
        if (options.resCache)
        {
          ...
          var firstScript = htmlTools.getElementByName(ast.head, 'script') || htmlTools.getElementByName(ast, 'script');
          if (firstScript)
            htmlTools.insertBefore(firstScript, resourceScript);
          else
  ...
  function normlizeOptions(key, value){
    switch (key){
      case 'jsSourceMapOffset':
  ...
  var httpServer = http.createServer(function(req, res){
    ...
    if(/\/setOptions/.test(req.url)){
      var newOptions = url.parse(req.url,true).query;

      Object.keys(newOptions).forEach(function(key){
        options[key] = normlizeOptions(key, newOptions[key]);
      });

      return res.end('');
    }

    //proxy request if necessary
    if (!location)
      return;
