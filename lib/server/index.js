...
function launch(config){
  ...
  files.addReadCallback(function(err, data, filename){
    ...
    {
      logMsg('info', file.filename + ' ' + chalk.yellow('(update content: ' + data.length + 'bytes)'));
  ...
  io.sockets.on('connection', function(socket){
    ...
    socket.on('ready', function(clientFiles){
      ...
      logMsg('socket', 'response ' + chalk.yellow('knownFiles') + ' (server ' + serverFiles.length + ' files)', true);
  ...
  var fsWatcher = (function(){
    ...
    function updateFile(fnKey){
      ...
      logMsg('watcher', fnKey + ' ' + chalk.yellow('(update file)'));
    ...
    function onWatchEvent(filename, dirInfo){
      logMsg('watcher', path.relative(options.base, filename) + ' ' + chalk.magenta('(event)'), true);
