...
function launch(config){
  ...
  // settings
  var readExtensions = ['.css', '.tmpl', '.txt', '.json', '.js', '.l10n'];
  var notifyExtensions = ['.css', '.tmpl', '.txt', '.json', '.l10n'];
  ...
  var virtualPath = {
    '/__basis_resources__.js': function(req, res){
      ...
      if (!contentIsNotModified(req, res, cache.mtime))
      {
        ...
        logMsg('client', '/__basis_resources__.js' + ' ' + chalk.yellow('(generate)'));
        ...
        logMsg('client', '/__basis_resources__.js' + ' ' + chalk.green('304'));
    }
  };
  ...
  function onFileRead(err, data, filename){
    if (err)
  ...
  function getFileInfo(filename){
    ...
    if (!file)
      file = fileMap[fnKey] = {
        filename: fnKey,
        hotStart: options.hotStartCacheByExt ? hotStartExtensions.indexOf(extname) != -1 : false,
  ...
  //
  if (options.runAcpServer)
  {
    require('./modules/appcp_server.js').create();
  }
  ...
  var httpServer = http.createServer(function(req, res){
    ...
    resolvePathnameFile(req, res, path.normalize(BASE_PATH + location.pathname), location, function(fres){
  ...
  httpServer.listen(options.port, function(){
    ...
    {
      if (options.sync)
        console.log('Watching changes for path: ' + chalk.green(BASE_PATH));
      else
  ...
  io.sockets.on('connection', function(socket){
    ...
    logMsg('socket', 'client connected');

    socket.on('disconnect', function(){
      logMsg('socket', 'client disconnected');
    });

    socket.on('watchFiles', function(files){
      logMsg('socket', 'request watchFiles (' + (files ? files.length : 0) + ' files)');
      if (Array.isArray(files))
        files.forEach(function(filename){
          fsWatcher.startWatch(path.normalize(BASE_PATH + '/' + filename));
        });
    });

    socket.on('saveFile', function(filename, content, autocreate, callback){
      logMsg('socket',
        'request saveFile ' + (autocreate ? '(autocreate)' : '') + filename +
    ...
    socket.on('readFile', function(filename, callback){
      logMsg('socket', 'request readFile ' + filename);
      ...
      if (fileInfo && typeof callback == 'function')
      {
        var sendData = {
          filename: fnKey,
      ...
      if (!fileInfo || !fileInfo.touched)
        files.readFile(path.normalize(BASE_PATH + '/' + filename), function(err, data){
          if (!err)
            return {
              filename: fnKey,
              lastUpdate: fs.statSync(filename).mtime,
              content: data
            };
        });
    });
    ...
    socket.on('openFile', function(filename, callback){
      logMsg('socket', 'request openFile ' + filename);
      ...
      if (options.editor)
      {
        var cmd = options.editor + ' ' + path.resolve(BASE_PATH, filename.replace(/^\//, ''));

        logMsg('cmd', 'openFile: ' + cmd);
    ...
    function resolveFilename(filename){
      ...
      if (fileStat.isDirectory())
      {
        if (fs.existsSync(filename + path.sep + 'index.html'))
          return filename + path.sep + 'index.html';

        if (fs.existsSync(filename + path.sep + 'index.htm'))
          return filename + path.sep + 'index.htm';
    ...
    socket.on('getFileGraph', function(filename, callback){
      logMsg('socket', 'request getFileGraph ' + filename);

      if (typeof callback == 'function')
      {
        var fn = resolveFilename(path.normalize(BASE_PATH + url.parse(filename, false, true).pathname));
        if (!fn)
          callback('File ' + filename + ' not found');

        var startTime = new Date;
        var cp = child_process.fork(__dirname + '/../extractor',
          ['--file', fn, '--base', BASE_PATH, '--js-cut-dev', '--css-info', '--l10n-info'],
          { silent: true }
        );
        ...
        cp.on('exit', function(code){
          if (code)
          {
            console.log('[WARN] getFileGraph: exit ' + code);
            callback('Process exit with code ' + code);
          }
        });

        cp.on('message', function(res){
          if (res.error)
          {
          ...
          {
            logMsg('socket', 'send file graph (extrating done in ' + (new Date - startTime) + 'ms)');
    ...
    socket.on('observe', function(rel, fspath){
      ...
      socket.emit('observeReady', result);
    });
  });

  var createCallback = function(fileInfo){
    logMsg('socket', 'broadcast newFile ' + fileInfo.filename);
    io.sockets.emit('newFile', fileInfo);
  }
  var updateCallback = function(fileInfo){
    logMsg('socket', 'broadcast updateFile ' + fileInfo.filename);
    io.sockets.emit('updateFile', fileInfo);
  }
  var deleteCallback = function(filename){
    logMsg('socket', 'broadcast deleteFile ' + filename);
  ...
  var fsWatcher = (function(){
    ...
    function onWatchEvent(filename){
      logMsg('fs', 'watch event for ' + filename, true);
    ...
    function getWatcher(dirInfo){
      ...
      logMsg('fs', chalk.red('start watch dir ') + path.relative(BASE_PATH, dirInfo.path).replace(/\\/g, '/'), true);        
      ...
      return fs.watch(dirInfo.path)
        ...
        .on('error', function(error){
          console.warn('FS watch error (' + path + '):' + error);
          if (!fs.existsSync(dirInfo.path))
          {
