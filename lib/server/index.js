...
function launch(config){
  ...
  var httpServer = http.createServer(function(req, res){
    ...
    resolvePathnameFile(req, res, path.normalize(options.base + location.pathname), location, function(fres){
      ...
      if (options.readCache)
      {
        ...
        if (fileInfo && (fsWatcher && fsWatcher.isFileObserve(filename)) && fileInfo.content != null)
        {
  ...
  (function(){
    ...
    syncServer.sockets.on('connection', function(socket){
      ...
      socket.on('saveFile', require('./modules/socket/saveFile.js')(options, broadcast));
      socket.on('createFile', require('./modules/socket/createFile.js')(options, broadcast));
      socket.on('readFile', require('./modules/socket/readFile.js')(options, broadcast));

      socket.on('openFile', require('./modules/socket/openFile.js')(options, broadcast));
      socket.on('getFileGraph', require('./modules/socket/getFileGraph.js')(options, broadcast));
      socket.on('handshake', require('./modules/socket/handshake.js')(options, broadcast));
    });
  ...
  var fsWatcher = require('./modules/watch');
}
