...
function launch(config){
  ...
  var virtualPath = require('./modules/virtualFile');
  ...
  var httpServer = http.createServer(function(req, res){
    ...
    resolvePathnameFile(req, res, path.normalize(options.base + location.pathname), location, function(fres){
      ...
      if (options.readCache)
      {
        ...
        var fileInfo = files.getInfo(fnKey);
  ...
  var syncServer = (function(){
    ...
    syncServer.sockets.on('connection', function(socket){
      ...
      socket.on('handshake', function(clientData){
        ...
        var serverFiles = files.getNames().filter(function(fn){
      ...
      socket.on('openFile', require('./modules/socket/openFile.js')(options));
      socket.on('getFileGraph', require('./modules/socket/getFileGraph.js')(options));
    });
  ...
  var fsWatcher = (function(){
    ...
    function updateFile(fnKey){
      var fileInfo = files.getInfo(fnKey, true);
          ...
          else
          {
            ...
            for (var fn in dirInfo.files)
              if (!fs.existsSync(dirInfo.files[fn]))
                files.dropInfo(dirInfo.files[fn]);
          }
        })
        ...
        dirInfo = watchFolders[path] = {
          ...
          add: function(filename){
            ...
            if (fnKey in this.files == false)
            {
              ...
              files.getInfo(filename, true); // reg file in global map
              this.count++;
