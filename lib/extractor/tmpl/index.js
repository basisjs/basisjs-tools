    ...
    for (var key in flow.tmpl.themeResources[themeName])
    {
      ...
      else
      {
        flow.warn({
          ...
          message: 'template ' + key + ' have no resourceRef, theme: ' + themeName
        });
      }
    }
    fconsole.endl();
      ...
      {  
        if (!resource)
        {
          if (cmpStr != baseDecl[key].str)
          {
            ...
            var impFile = flow.files.add({
              jsRefCount: 1,
              type: 'template',
              isResource: true
            });
            impFile.filename = genericFilename;
            implicitDefine[themeName][key] = impFile;
