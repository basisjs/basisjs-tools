...
function processFile(file, flow){
  ...
  try {
    ...
    console.warn('[FATAL ERROR] Parse error of ' + file.relpath + (' (line: ' + e.line + ', col: ' + e.col + ')') + ':\n' + (e.message || e));
    process.exit();
  }
  ...
  else
  {
      ...
      var getTheme = function(name){
        ...
        var theme = {
          ...
          define: at.createRunner(function(token, this_, args){
            ...
            if (!what && args[0])
            {
              fconsole.log('[ERROR] basis.template.define: first parameter is not resolved', at.translate(token));
              process.exit();
            }

            if (!by && args[1])
            {
              fconsole.log('[WARN] basis.template.define: second parameter is not resolved', at.translate(token));
              process.exit();
            }
    ...
    if (file.namespace == 'basis.l10n')
    {
      ...
      defineHandler(file.jsScope, 'basis.l10n.createDictionary', function(token, this_, args){
        ...
        if (!name)
        {
          fconsole.log('[ERROR] basis.l10n.createDictionary: first parameter is not resolved', at.translate(token));
          process.exit();
        }

        if (!dir)
        {
          fconsole.log('[ERROR] basis.l10n.createDictionary: second parameter is not resolved', at.translate(token));
          process.exit();
        }

        if (!keys)
        {
          fconsole.log('[ERROR] basis.l10n.createDictionary: third parameter is not resolved', at.translate(token));
          process.exit();
        }
        ...
        for (var key in keys)
        {
          ...
          if (!val || val[0] != 'string')
          {
            fconsole.log('[ERROR] basis.l10n.createDictionary: value is not resolved (third parameter)', at.translate(val), at.translate(token));
            process.exit();
          }
          keysObj[key] = val[1];
        }
