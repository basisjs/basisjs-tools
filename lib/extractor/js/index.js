      ...
      var getTheme = function(name){
        ...
        var theme = {
          ...
          define: at.createRunner(function(token, this_, args){
            ...
            var by = this.scope.simpleExpression(args[1]);
    ...
    if (file.namespace == 'basis.l10n')
    {
      ...
      if (file.jsScope.resolve(at.parse('basis.l10n.dictionary', 1)))
      {
        ...
        var resolveL10nToken = function(key, dictPath){
          var id = key + '@' + dictPath;
          var tokenDescriptor = flow.l10n.tokens[id];
          var token = tokenDescriptor && tokenDescriptor.token;
          ...
          if (!token)
          {
            ...
            token.obj = {
              compute: at.createRunner(function(token_, this_, args){
                ...
                token_.obj = resolveL10nToken(key + '.{?}', dictPath);
              })
            };

            flow.l10n.tokens[key] = {
              token: token
            };
          }
        ...
        var resolveDictionary = function(filename){
          var dict = flow.l10n.dictList[filename];
          ...
          if (!dict)
          {
            ...
            token.obj = {
              token: at.createRunner(function(token_, this_, args){
                ...
                if (key && key[0] == 'string')
                {
                  ...
                  token_.obj = resolveL10nToken(key[1], file.relpath).obj;
                  dict.usedTokens[key[1]] = true;
                  //getTokenList.push(token);
                }
                else
                {
                  flow.warn({
            ...
            dict = flow.l10n.dictList[filename] = {
              token: token,
              file: file,
              tokens: [],
              usedTokens: {}
        ...
        defineHandler(file.jsScope, 'basis.l10n.dictionary', function(token, this_, args){
          ...
          dict.tokens.push({ type: 'js', token: token });
          dict.file.jsRefCount++;
          token.obj = dict.token.obj;
