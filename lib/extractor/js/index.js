...
function createScope(file, flow, module){
  if (file.type == 'script' && !file.jsScope)
  {
    ...
    var names = {
      ...
      __dirname: ['string', flow.js.oldBasis
        ? file.baseURI
        : file.baseURI.replace(/\/$/, '')
      ],
      ...
      resource: at.createRunner(function(token, this_, args){
        ...
        var uri = args[0][0] == 'string' ? args[0][1] : at.getCallArgs(args, this.file.context, flow, this.file)[0];
        if (uri)
        {
          token[1] = ['dot', ['name', 'basis'], 'resource'];
          token[1].ref_ = basisResource;
          token[1].refPath_ = 'basis.resource';
          token[2] = [args[0] = ['string', this.file.resolve(uri)]];
...
function processFile(file, flow){
  ...
  try {
    switch (file.namespace)
    {
      case 'basis':
        ...
        global.basis.config.extProto = false;
        Object.keys = keys; // Object.keys overload fix  

        // omit dev output
        basis.dev.log = function(){};
        basis.dev.info = function(){};
        // override dev warn output
        basis.dev.warn = function(){
          flow.warn({
            message: ['basis.js warning:'].concat(Array.prototype.slice.call(arguments))
          });
        };
      break;
  ...
  if (file.basisScript)
  {
    ...
    var createNS = function(path){
      ...
      token.run = function(token_, this_, args){
        ...
        if (token.setWrapper_)
        {
          ...
          {
            // temporary commented, until handlers for wrap functions
            /*flow.warn({
              file: this.file.relpath,
              message: 'namespace ' + path + ' was called, but namespace wrapper has no run handler'
            });*/
          }
        }
        else
        {
          ...
          /*flow.warn({
            file: this.file.relpath,
            message: 'namespace ' + path + ' was called, but namespace has no wrapper'
          });*/
        }
      }
    ...
    function getResourceRunner(token, this_, args){
      ...
      var flowArg = /^basis\.(template|l10n)$/.test(this.file.namespace) ? { warn: function(){} } : flow;

      args = token[2]; // FIXME: it's a hack
      var newFilename = args[0][0] == 'string'
        ? args[0][1]
        : at.getCallArgs(args, this.file.context, flowArg, this.file)[0];
      ...
      if (newFilename)
      {
        ...
        var newFile = flow.files.add({
          filename: path.resolve(path.dirname(flow.options.file), newFilename),
          jsRefCount: 0
        });
      ...
      requireNamespace: function(token, this_, args){
        ...
        if (args[0] && args[0][0] == 'string')
        {
          ...
          else
          {
            ...
            var filename = path.resolve(path.dirname(flow.options.file), arg1, arg0); // TODO: resolve filename relative to html file
            var file = this.file;
    ...
    if (file.autoload)
    {
      var autoRequire = at.parse('basis.require("' + file.autoload + '")', true);
      //file.ast[1][1][1][1][3].push(autoRequire);
      handlers.requireNamespace.call({ file: file }, autoRequire, null, [['string', file.autoload]]);
      flow.js.autoload = autoRequire;
    }
  }
  else
  {
      ...
      if (file.jsScope.resolve(at.parse('basis.l10n.dictionary', 1)))
      {
          ...
          if (!dict.jsToken)
          {
            ...
            token.obj = {
              token: at.createRunner(function(token_, this_, args){
                ...
                if (key && key[0] == 'string')
                {
                  var id = key[1] + '@' + file.relpath;
                  fconsole.log('[basis.l10n] basis.l10n.Dictionary#token ' + id);

                  var l10nToken = resolveL10nToken(key[1], file.relpath);
        ...
        defineHandler(file.jsScope, 'basis.l10n.dictionary', function(token, this_, args){
          ...
          if (filename && filename[0] == 'string')
          {
            ...
            fconsole.log('[basis.l10n] dictionary ' + at.translate(args[0]) + ' -> ' + filename);
