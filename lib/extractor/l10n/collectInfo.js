...
(module.exports = function(flow){
  ...
  flow.files.queue.forEach(function(file){
    if (file.type == 'template')
    {
      ...
      tmplAt.walk(file.ast, {
        text: function(token){
          if (l10nPrefix.test(token[1]))
          {
            fconsole.log(token[1]);
            tmplRefs.push({
              file: this.file,
              key: token[1].substr(5),
              host: token,
              idx: 1
            });
          }
  ...
  for (var i = 0, entry; entry = defList[i]; i++)
  {
    ...
    {
      nameFile[name] = file;
      dictionaries[name] = dictpath;
      locations[name] = path.relative(flow.options.base, dictpath);
    }
