...
function here(options){
  ...
  else
  {
    if (fs.existsSync('basisjs'))
    {
      ...
      console.log(basisjsPath);
      isPathBasisRepo(basisjsPath, function(){
        console.log('I found basis.js repo on current location (basisjs folder)');
        ...
        console.log('I found basisjs folder on current location, but i\'t not a basis.js repo. Delete it first and try again.', error);
      });
    }
    else
      checkForEnv();
...
function setBasisjsEnv(path){
  ...
  exec(cmd, function(error, stdout, stderr){
    ...
    console.log('\nBASISJS_PATH set successful to ' + path, stdout);
  });
}
