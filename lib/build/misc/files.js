  ...
  */
  add: function(data){
    ...
    else
    {
      ...
      {
        this.console.log('[skip] ' + unixpath(path.relative(this.baseURI, filename)) + ' (already in queue)');
      ...
      if (fs.existsSync(filename))
      {
        if (fs.statSync(filename).isFile())
        {
          ...
          this.console.start('[+] ' + file.relpath + ' (' + file.type + ')' + (preprocessors.length ? ' [w/ preprocessing]' : ''));
          content = fs.readFileSync(filename, file.encoding);
  ...
  */
  remove: function(fileRef){
    ...
    else
    {
      ...
      if (!file)
      {
        this.console.log('[WARN] File `' + fileRef + '` not found in map');
        return;
      }
    }
