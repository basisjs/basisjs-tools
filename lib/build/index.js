...
var fetchCommit = require('./misc/fetch-commit');
...
function getToolsId(){
  ...
  if (!toolsId)
  {
    var commit = fetchCommit(__dirname + '/../..');
    toolsId = toolsPkg.name + '@' + toolsPkg.version + (commit ? ' (' + commit + ')' : '');
  }
...
function build(config){
  ...
  process.basisjsBaseURI = path.dirname(options.file);
