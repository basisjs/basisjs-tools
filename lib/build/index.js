...
function build(config){
  ...
  else
  {
    process.stdout.write(
      'Index file: ' +
        chalk.green(path.relative(process.cwd(), options.file).replace(/^([^\.\/])/, './$1')) + '\n' +
      'Output: ' +
        chalk.green(path.relative(process.cwd(), options.output).replace(/^([^\.\/])/, './$1') || '<current folder>') + '\n' +
      '\n'
    );
  }
  ...
  // set working dir to index file location
  if (process.cwd() != inputDir)
  {
    fconsole.log('Change working dir to ' + inputDir);
