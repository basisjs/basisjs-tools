...
function wrapPackage(package, flow, contentPrepend){
  ...
  return [
    ...
    packageWrapper[0],
    contentPrepend.replace('(requestUrl)', '(__ns2fn[namespace] || requestUrl)'), // FIXME: this is dirty to patch old basis version (prior to 0.10)

    ';var __ns2fn = {}',
    ';basis.require = (' + (function(map){
      var _require = basis.require;
      for (var key in map)
        __ns2fn[map[key]] = basis.path.resolve(key);
      return function(fn){
        return _require(map[fn]);
      };
    }).toString() + ')(' + JSON.stringify(flow.js.fn2ns) + ')',
