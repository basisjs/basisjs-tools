...
function processFile(file, flow){
  ...
  if (file.basisScript)
  {
    ...
    var createClass = function(context, args){
      // var prototype = ['object', []];
      // var className = 'UnknownClassName';

      // for (var i = 0; i < args.length; i++)
      //   if (args[i] && args[i][0] == 'object')
      //   {
      //     if (args[i].obj.className)
      //       className = args[i].obj.className;
      //     astExtend(context.scope, prototype, args[i]);
      //   }

      fconsole.log('[basis.Class.create]');

      var token = ['function', null, []];
      token.obj = {
        //prototype: prototype,
        subclass: at.createRunner(function(token, this_, args){
          token.obj = createClass(this, args).obj;
        })
      };
      return token;
    }
    ...
    // basis.Class.create
    defineHandler(globalScope, 'basis.Class.create', function(token, this_, args){
      token.obj = createClass(this, args).obj;
    });
