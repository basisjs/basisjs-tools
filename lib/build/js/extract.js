  ...
  if (file.type == 'script' && !file.jsScope)
  {
    ...
    var names = {
      ...
      resource: at.createRunner(function(token, this_, args){
        ...
        if (uri)
        {
          args[0] = ['string', file.baseURI + '/' + uri];
...
function defineHandler(scope, name, fn, createRunner){
  if (name.indexOf('.') != -1)
  {
...
function processScript(file, flow){
  ...
  if (file.basisScript)
  {
    var basisScope = ast.scope.subscopes[ast.scope.subscopes.length - 1];
  ...
  else
  {
    ...
    if (file.namespace == 'basis.template')
    {
      var getTheme = function(name){
        ...
        if (flow.js.themes[name])
          return flow.js.themes[name];

        var fn = function(name){
          return at.createRunner(function(token, this_, args){
            console.log('basis.template#method', name, args);
          });
        };

        function addSource(key, value){
          console.log('[basis.template] define template ', key, 'in', theme.name, 'theme');
          resources[key] = value;
          return value;
        }
        ...
        flow.js.themes[name] = theme;
        flow.js.themeResources[name] = resources;
