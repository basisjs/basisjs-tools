...
function processScript(file, flow){
  ...
  if (file.basisScript)
  {
    ...
    /*var requireNamespace = basisScope.get('requireNamespace');
    requireNamespace.token = ['function', null, []];
    requireNamespace.token.run = function(token, t, args){
          ...
          case BASIS_REQUIRE:
            ...
            if (namespace)
            {
              ...
              if (!rootFile)
              {
                ...
                globalScope.put(root, 'ns', createNS(root)); // ?? remove
              }
