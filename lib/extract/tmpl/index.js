...
(module.exports = function(flow){
  ...
  themeOrdered.forEach(function(themeName){
    ...
    for (var defineIdx = 0, key; key = defineKeys[defineIdx]; defineIdx++)
    {
      ...
      // l10nTokens was implemented during developing 1.4 and renamed to l10n back before release
      // TODO: remove after basis.js 1.4 released
      if (!l10nTokens && decl.l10nTokens)
        l10nTokens = Object.keys(decl.l10nTokens);
                ...
                hasOwnProperty.call(stylesMap, resourceFilename))
            {
              ...
              if (styleInfo.constructor === Object)
              {
                ...
                if (styleInfo.isolate)
                {
                  ...
                  // if (styleInfo.namespace && styleToken.sourceUrl)
                  // {
                  //   var loc = [
                  //     styleToken.sourceUrl,
                  //     styleToken.loc.start.line + 1,
                  //     styleToken.loc.start.column + 1
                  //   ].join(':');

                  //   if (!resFile.namespacedBy)
                  //     resFile.namespacedBy = [loc];
                  //   else
                  //     arrayAddUnique(resFile.namespacedBy, loc);
                  // }
                }
