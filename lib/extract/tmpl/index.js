...
function copyWarnsToFlow(warns, knownNestedWarnings, flow, themeName, filename){
  if (warns)
...
(module.exports = function(flow){
  ...
  themeOrdered.forEach(function(themeName){
    ...
    for (var defineIdx = 0, key; key = defineKeys[defineIdx]; defineIdx++)
    {
      ...
      if (!resource && !fallbackDecl[fallbackTheme].hasOwnProperty(key))
        continue;
      ...
      if (file)
      {
        ...
        if (decl.resources.length)
        {
          for (var j = 0, resourceFilename; resourceFilename = decl.resources[j]; j++)
          {
            ...
            if (Array.isArray(decl.styles) && decl.styles[j] && typeof decl.styles[j][1] == 'string')
              resFile.isolate = decl.styles[j][1];
