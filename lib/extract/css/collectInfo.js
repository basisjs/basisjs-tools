...
(module.exports = function(flow){
    ...
    switch (file.type){
      ...
      case 'template':
        ...
        if (file.ast)
        {
          ...
          if (file.removals)
            file.removals.forEach(function(fragment){
              console.log('!!!!ura', !!fragment.token);
              processTemplateAst(file, [fragment.token], idMapRemovals, classMapRemovals);
            });
        }
  ...
  for (var theme in classMap)
    for (var name in classMap[theme])
    {
      ...
      var removedFromHtml = removalsList && (list.sources.html || list.sources.tmpl);

      console.log(name, !!removedFromHtml);
